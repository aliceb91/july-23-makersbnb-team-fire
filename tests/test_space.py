...
def test_creates_space():
    ...
    space = Space(1, "Test Space", "Test Description", 10.0, datetime.date(2000, 1, 1), datetime.date(2000, 1, 2),1)
    assert space.id == 1
    assert space.name == "Test Space"
    assert space.description == "Test Description"
    assert space.price_per_night == 10.0
    assert space.user_id == 1
    assert space.start_date == datetime.date(2000, 1, 1)
    assert space.end_date == datetime.date(2000, 1, 2)

def test_compares_to_similar_object():
    # Given two space objects
    # It compares their contents
    space_1 = Space(1, "Test Space", "Test Description", 10.0, datetime.date(2000, 1, 1), datetime.date(2000, 1, 2), 1)
    space_2 = Space(1, "Test Space", "Test Description", 10.0, datetime.date(2000, 1, 1), datetime.date(2000, 1, 2), 1)
...
def test_outputs_string_when_called():
    ...
    space = Space(1, "Test Space", "Test Description", 10.0, datetime.date(2000, 1, 1), datetime.date(2000, 1, 2), 1)
    assert str(space) == f"Space(1, Test Space, Test Description, 10.0, {datetime.date(2000, 1, 1)}, {datetime.date(2000, 1, 2)}, 1)"

def test_create_date_range():
    # Given a Space object
    # It creates a range of dates between a given start and end date.
    space = Space(1, "Test Space", "Test Description", 10.0, datetime.date(2000, 1, 6), datetime.date(2000, 1, 10), 1)
